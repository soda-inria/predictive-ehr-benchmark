...
REGISTRY_NAMESPACE = has-sante
PROJECT_DIR := $(shell dirname $(realpath $(lastword $(MAKEFILE_LIST))))
PROJECT_NAME := $(shell basename $(PROJECT_DIR))
DOCKER_REPOSITORY = $(REGISTRY_NAMESPACE)/$(PROJECT_NAME)
PAPER_DIR = $(PROJECT_DIR)/../papiers/event2vec_paper/img/
...
# Custom certs may be used on the HAS infrastructure and requests needs to be
# aware of them
...
## `pyproject.toml`, and install event2vec source code
update-dependencies:
	poetry update --lock
ifneq (${IS_CI}, true)
...
## Test via tox in poetry env
coverage: clean
	poetry run pytest --cov=event2vec tests/
...
pre-commit:
	poetry run pre-commit run $(PRECOMMIT_HOOK_ID) --all-files
...
.PHONY: cp-img
cp-img:
	cp -r $(PROJECT_DIR)/docs/source/_static/img/experiences/timesplit__complete_hospitalization_los__age_min_18__dates_2017_2022__task__length_of_stay_categorical@3_all_featurizers/* $(PAPER_DIR)/los/
	cp -r $(PROJECT_DIR)/docs/source/_static/img/experiences/timesplit__icd10_prognosis__age_min_18__dates_2017_2022__task__prognosis@cim10lvl_1__rs_0__min_prev_0.01_four_ML/* $(PAPER_DIR)/prognosis/
	cp -r $(PROJECT_DIR)/docs/source/_static/img/experiences/timesplit__mace__age_min_18__dates_2018_2020__task__MACE@360__index_visit_random_four_ML/* $(PAPER_DIR)/mace/
