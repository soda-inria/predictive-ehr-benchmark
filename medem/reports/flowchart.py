...
from medem.constants import DIR2DOCS_COHORT


def get_flowchart_from_inclusion_ids(
    inclusion_ids: Dict[str, List[str]],
    flowchart_name: str,
    target_name: str = None,
    cohort_characteristics: pd.DataFrame = None,
    dir2cohort: Path = None,
):
    F = Flowchart(
        data=inclusion_ids,
        initial_description="Initial population",
        cohort_characteristics=cohort_characteristics,
        concat_criterion_description=False,
    )
    for criterion_name in inclusion_ids.keys():
        if criterion_name not in ["initial", target_name]:
            F.add_criterion(
                criterion_name=criterion_name,
                description="",
                excluded_description=criterion_name,
            )
    if target_name is not None:
        F.add_final_split(
            criterion_name=target_name,
            left_description=target_name,
            right_description=f"No {target_name}",
            # left_title="Treated",
            # right_title="Control",
        )
    F.generate_flowchart(alternate=False)
    if dir2cohort is None:
        dir2cohort = DIR2DOCS_COHORT
    dir2cohort.mkdir(exist_ok=True, parents=True)
    F.save(dir2cohort / (flowchart_name + ".png"))
    F.save(dir2cohort / (flowchart_name + ".svg"))
    return F
