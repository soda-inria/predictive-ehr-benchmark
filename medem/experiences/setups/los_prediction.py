...
def run_experience(config: Dict[str, str] = None):
    ...
    """
    config_cohort = CONFIG_LOS_COHORT
    config_experience = CONFIG_LOS_ESTIMATION
    ...
    for run_config in run_to_be_launch:
        ...
        train_prevalence = train_person[COLNAME_OUTCOME].mean()

        if config_experience.get("positive_ratio", None) is not None:
            estimator.set_params(
                **{
                    "class_weight": {
                        0: 1 - train_prevalence,
                        1: train_prevalence,
                    }
                }
            )
        ...
        train_prevalence = {
            "train_prevalence": 100 * subtrain_person[COLNAME_OUTCOME].mean()
        }
        ...
        test_prevalence = {
            "test_prevalence": 100 * test_person[COLNAME_OUTCOME].mean()
        }
        # validation scores
        if config_experience["validation_size"] is not None:
